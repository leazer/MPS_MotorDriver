...
### 调试记录规范 (每个 Stage 必须遵守)

每个 Stage 的开发与台架调试过程必须记录到 `doc/调试记录.md`, 用于后续比赛文档和视频素材. 记录要求:
- 每个 Stage 完成后追加一节, 含: 背景 / 关键决策 / 实现要点 / 台架验证结果 / 踩坑总结
- 调试过程中遇到的 bug 及修复过程单独记录 (现象 / 排查 / 根因 / 修复 / 结果), 哪怕是 1 行代码的修复
- 台架验证结果用表格列出验证项 / 命令或仪器 / 结果
- 新增的 msh 命令记录到 CLAUDE.md 的命令清单
